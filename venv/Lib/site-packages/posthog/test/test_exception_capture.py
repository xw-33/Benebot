...
def test_excepthook(tmpdir):
    app = tmpdir.join("streamlit_app.py")
    app.write(
        dedent(
            """
    ...
    assert (
        b'"$exception_list": [{"mechanism": {"type": "generic", "handled": true}, "module": null, "type": "ZeroDivisionError", "value": "division by zero", "stacktrace": {"frames": [{"platform": "python", "filename": "streamlit_app.py", "abs_path"'
        in output
    )
